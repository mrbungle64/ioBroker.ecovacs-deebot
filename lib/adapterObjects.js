...
async function createInitialObjects(adapter) {
    if (adapter.getModelType() === 'aqMonitor') {
        return;
    }
    ...
    if (adapter.vacbot.hasSpotAreaCleaningMode()) {
        await adapter.createObjectNotExists(
            ...
            'string', 'value', true, '', '');
        if ((adapter.getModelType() === 'legacy') || (adapter.getModelType() === '950')) {
            await adapter.createObjectNotExists(
        ...
        if (adapter.getModel().isSupportedFeature('control.goToPosition') && adapter.getDevice().useNativeGoToPosition()) {
            await adapter.createObjectNotExists(
        ...
        await adapter.deleteObjectIfExists('control.spotArea_cleanings');
        if (adapter.getModelType() !== 'airbot') {
    ...
    if (adapter.getModel().hasCustomAreaCleaningMode() || adapter.getModel().isModelTypeAirbot()) {
        if (adapter.getModel().isSupportedFeature('control.goToPosition') || adapter.getDevice().useNativeGoToPosition()) {
            await adapter.createObjectNotExists(
    ...
    if (adapter.getModel().hasCustomAreaCleaningMode()) {
    ...
    if ((adapter.getModel().isSupportedFeature('control.volume')) && (adapter.getModelType() !== 'airbot')) {
        await adapter.createObjectNotExists(
        ...
        await changeObjName(adapter, 'control.extended.volume', 'Volume for voice and sounds (0-10)');
    } else {
        await adapter.deleteObjectIfExists('control.extended.volume');
    }
    if (adapter.getModel().hasAdvancedMode()) {
        await adapter.createObjectNotExists(
    ...
    if (adapter.getModel().vacbot.getDeviceProperty('hosted_mode')) {
        await adapter.createObjectNotExists(
    ...
    await adapter.createChannelNotExists('control.extended.genericCommand', 'Run generic command (Experienced users only!)');
    await adapter.createObjectNotExists(
    ...
    //
    if ((adapter.getModelType() !== 'goat') && (adapter.getModelType() !== 'aqMonitor')) {
        ...
        if (adapter.getModelType() !== 'airbot') {
            ...
            if (adapter.getModel().isSupportedFeature('consumable.airFreshener')) {
                await adapter.createObjectNotExists(
...
async function createControlSweepModeIfNotExists(adapter, states) {
    if (!adapter.vacbot.hasMoppingSystem()) {
        return;
    }
    await adapter.setObjectNotExists('control.extended.moppingMode', {
        'type': 'state',
        'common': {
            'name': 'Mopping mode',
            'type': 'number',
...
async function createControlScrubbingPatternIfNotExists(adapter, states) {
    if (!adapter.vacbot.hasMoppingSystem() || !adapter.vacbot.hasRoundMopInfo()) {
        return;
    }
    await adapter.setObjectNotExists('control.extended.scrubbingPattern', {
        'type': 'state',
        'common': {
            'name': '`Scrubbing pattern (OZMO Pro and round mops)`',
            'type': 'number',
            'role': 'level',
            'read': true,
            'write': true,
...
async function createControlWashIntervalIfNotExists(adapter) {
    const isT20Type = adapter.getModel().isModelTypeT20() || adapter.getModel().isModelTypeX2();
...
async function createAdditionalObjects(adapter) {
    ...
    if (adapter.getModel().isSupportedFeature('cleaninglog.channel')) {
        ...
        await adapter.deleteObjectIfExists('cleaninglog.requestCleaningLog');
...
async function changeObjName(adapter, objId, name) {
    const obj = await adapter.getObjectAsync(objId);
    if (obj && obj.common) {
        obj.common.name = name;
        await adapter.extendObjectAsync(objId, obj);
    }
}
