...
async function createInitialObjects(adapter) {
    ...
    if (adapter.vacbot.hasSpotAreas()) {
        await adapter.createObjectNotExists(
            ...
            'string', 'value', true, '', '');
        if (adapter.getModel().isSupportedFeature('control.spotArea_cleanings')) {
            await adapter.createObjectNotExists(
    ...
    await adapter.createObjectNotExists(
        'control.reconnectAPI', 'Reconnect Ecovacs API',
        'boolean', 'button', true, false, '');
...
async function createControlWaterLevelIfNotExists(adapter, def = 2, id = 'control.waterLevel', name = 'Water level') {
    ...
    if ((id !== 'control.waterLevel') && !adapter.getModel().isSupportedFeature('map.spotAreas.waterLevel')) {
        return;
    }
    const states = {
        1: 'low',
...
async function createControlCleanSpeedIfNotExists(adapter, def = 2, id = 'control.cleanSpeed', name = 'Clean speed') {
    if ((id !== 'control.cleanSpeed') && !adapter.getModel().isSupportedFeature('map.spotAreas.cleanSpeed')) {
        return;
    }
    const states = {
        1: 'silent',
...
async function createExtendedObjects(adapter) {
    ...
    if (adapter.getModel().isSupportedFeature('info.ip')) {
        await adapter.createChannelNotExists('info.network', 'Network information');
        await adapter.createObjectNotExists(
            'info.network.ip', 'IP address',
            'string', 'text', false, '', '');
        if (adapter.getModel().isSupportedFeature('info.wifiSSID')) {
            await adapter.createObjectNotExists(
                'info.network.wifiSSID', 'WiFi SSID',
                'string', 'text', false, '', '');
        }
        if (adapter.getModel().isSupportedFeature('info.wifiSignal')) {
            await adapter.createObjectNotExists(
                'info.network.wifiSignal', 'WiFi signal strength in dBm',
                'number', 'level', false, '', 'dBm');
        }
        if (adapter.getModel().isSupportedFeature('info.mac')) {
            await adapter.createObjectNotExists(
