...
async function createInitialObjects(adapter, ctx) {
    ...
    if (ctx.vacbot.hasSpotAreaCleaningMode()) {
        ...
        await ctx.adapterProxy.deleteObjectIfExists('control.spotArea_cleanings');
        if (ctx.getPlatformType() !== 'airbot' && ctx.getPlatformType() !== 'goat') {
    ...
    //
    if ((ctx.getPlatformType() !== 'goat') && (ctx.getPlatformType() !== 'aqMonitor')) {
...
async function createAdditionalObjects(adapter, ctx) {
    ...
    if (ctx.getModel().is950type_V2() && platformType !== 'goat') {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().is950type_V2() && ctx.vacbot.hasMoppingSystem() && platformType !== 'goat') {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().is950type() && ctx.vacbot.hasMoppingSystem() && platformType !== 'goat') {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().isModelTypeT9Based() && platformType !== 'goat') {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if ((ctx.getModel().isModelTypeT20() || ctx.getModel().isModelTypeX2() || ctx.getModel().isModelTypeX1()) && platformType !== 'goat') {
        await ctx.adapterProxy.createObjectNotExists(
