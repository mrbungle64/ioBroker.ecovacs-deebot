...
async function createInitialObjects(adapter) {
    ...
    if (adapter.vacbot.hasCustomAreaCleaningMode()) {
        ...
        if (adapter.getModel().isSupportedFeature('control.resetCleanSpeedOnReturn') &&
            adapter.getModel().isSupportedFeature('control.cleanSpeed')) {
            await adapter.createObjectNotExists(
                'control.extended.resetCleanSpeedOnReturn', 'Always reset clean speed to standard on return',
                'boolean', 'value', true, false, '');
        } else {
            await adapter.deleteObjectIfExists('control.extended.resetCleanSpeedOnReturn');
        }
        await adapter.createObjectNotExists(
    ...
    if (adapter.vacbot.hasUnitCareInfo()) {
        await adapter.createObjectNotExists(
            'consumable.unit_care', 'Unit care lifespan',
            'number', 'level', false, 100, '%');
    }
