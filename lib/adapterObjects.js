...
async function createInitialObjects(adapter) {
    if (adapter.getModel().getModelType() === 'aqMonitor') {
        return;
    }
    ...
    if (adapter.vacbot.hasSpotAreaCleaningMode()) {
        await adapter.createObjectNotExists(
            ...
            'string', 'value', true, '', '');
        if ((adapter.getModel().getModelType() === 'legacy') || (adapter.getModel().getModelType() === '950')) {
            await adapter.createObjectNotExists(
    ...
    if (adapter.vacbot.hasCustomAreaCleaningMode() || (adapter.getModel().getModelType() === 'airbot')) {
        if (adapter.getModel().isSupportedFeature('control.goToPosition') || adapter.getDevice().useNativeGoToPosition()) {
            await adapter.createObjectNotExists(
    ...
    if ((adapter.getModel().isSupportedFeature('control.volume')) && (adapter.getModel().getModelType() !== 'airbot')) {
        await adapter.createObjectNotExists(
    ...
    if (adapter.getModel().is950type() &&
        adapter.getModel().isMappingSupported() &&
        adapter.getModel().getModelType() !== 'X1' &&
        adapter.getModel().getModelType() !== 'airbot') {
        await adapter.createObjectNotExists(
    ...
    await adapter.createObjectNotExists(
        ...
        'boolean', 'button', true, false);
    await adapter.createObjectNotExists(
        'control.extended.genericCommand.lastResponse', 'Last response (payload)',
        'json', 'value', false, '');
    ...
    //
    if ((adapter.getModel().getModelType() !== 'goat') && (adapter.getModel().getModelType() !== 'aqMonitor')) {
...
async function createControlScrubbingPatternIfNotExists(adapter, def = 1, id = 'control.extended.scrubbingPattern', name = 'Scrubbing pattern (OZMO Pro and round mops)') {
    if (!adapter.vacbot.hasMoppingSystem()) {
        return;
    }
    const hasRoundMops = adapter.vacbot.hasRoundMopInfo();
    const states = {
        1: hasRoundMops ? 'standard' : 'quick scrubbing',
        2: hasRoundMops ? 'deep' : 'deep scrubbing'
    };
    await adapter.setObjectNotExists(id, {
        'type': 'state',
        'common': {
