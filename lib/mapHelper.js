...
async function saveLastUsedSpotAreaValues(adapter) {
    ...
    const state = await adapter.getStateAsync('map.lastUsedSpotAreaValues');
    if (state && state.val) {
        await adapter.createChannelNotExists('map.savedCustomAreas', 'Saved areas');
        ...
        const obj = adapter.getObjectAsync('control.spotArea');
        if (obj && obj.native && obj.native.dateTime && obj.native.currentMapID) {
...
function cleanSavedSpotArea(adapter, id) {
    ...
    if (pattern.test(id)) {
        adapter.getObjectAsync(id).then(obj => {
            if (obj && obj.native && obj.native.area) {
                adapter.cleaningQueue.run('SpotArea', obj.native.area);
            }
        });
    }
}
