                ...
                adapter.getObject('map.' + extMapId, function (err, mapObj) {
                    if (mapObj) {
                        ...
                        if (!model.isSupportedFeature('map.virtualBoundaries')) {
                            ...
                            adapter.createChannelNotExists('map.' + extMapId + '.virtualBoundaries.availableBoundaries', 'Active virtual boundaries in the map');
                            adapter.createChannelNotExists('map.' + extMapId + '.virtualBoundaries.savedBoundaries', 'Saved virtual boundaries in the map for de-/activation');
                        }
                    }
                    delete mapArray[extMapId];
                });
...
function processVirtualBoundaries(adapter, virtualBoundaries) {
    ...
    if (typeof virtualBoundaries != 'object') {
        adapter.log.debug("[processVirtualBoundaries] Wrong parameter type for virtualBoundaries: "+typeof virtualBoundaries);
        return false;
    }
    let virtualBoundariesCombined = [...virtualBoundaries['mapVirtualWalls'], ...virtualBoundaries['mapNoMopZones']];
    ...
    for (const i in virtualBoundariesCombined) {
        if (Object.prototype.hasOwnProperty.call(virtualBoundariesCombined, i)) {
            adapter.log.debug("[processVirtualBoundaries] Pushing for map "+mapID+" virtualBoundary "+virtualBoundaries["mapVirtualWalls"][i]["mapVirtualBoundaryID"]);
            virtualBoundaryArray[virtualBoundariesCombined[i]['mapVirtualBoundaryID']] = virtualBoundariesCombined[i];
            adapter.log.debug("[processVirtualBoundaries] Sending GetVirtualBoundaryInfo command for map "+mapID+" virtualBoundary "+virtualBoundaries["mapVirtualWalls"][i]["mapVirtualBoundaryID"]);
        ...
        for (const r in channel) {
            ...
            if (!virtualBoundaryObj._id.includes('virtualBoundaries') || !virtualBoundaryObj._id.includes(mapID)) { // only process subchannels of virtualBoundaries.availableBoundaries of specified map
                ...
                if (!virtualBoundary) { //not existent (anymore)
                    adapter.deleteChannel(virtualBoundaryObj._id);
        ...
        for (const extVirtualBoundaryId in virtualBoundaryArray) { //create new states
            adapter.getObject('map.' + mapID + '.' + extVirtualBoundaryId, function (err, virtualBoundaryObj) {
                if (virtualBoundaryObj) {
                    adapter.log.debug("[processVirtualBoundaries] virtualBoundary object already existing: "+extVirtualBoundaryId);
                    adapter.setStateChanged('map.' + mapID + '.virtualBoundaries.' + extVirtualBoundaryId + '.virtualBoundaryIsAvailable', true, true);
                    adapter.setStateChanged(virtualBoundaryObj._id + '.virtualBoundaryDeactivationTimestamp', null, true);
                    ...
                    adapter.createObjectNotExists('map.' + mapID + '.virtualBoundaries.' + extVirtualBoundaryId + '.virtualBoundaryIsAvailable', 'Is the VirtualBoundary still available?', 'boolean', 'indicator.status', false, true, '');
                    ...
                    adapter.getObject('map.' + mapID + '.virtualBoundaries.' + extVirtualBoundaryId + '.virtualBoundaryCoordinates', function (err, obj) {
                        if (!obj) {
                            adapter.createObjectNotExists('map.' + mapID + '.virtualBoundaries.' + extVirtualBoundaryId + '.virtualBoundaryCoordinates', 'Coordinate of the virtualBoundary 2 or 4 pairs of x,y in [] defining a line or a rectangle', 'string', 'text', false, null, '');
                        }
                    });
                    adapter.createObjectNotExists(
                        ...
                        'boolean', 'button', true, false, '');
                    adapter.createObjectNotExists(
                        'map.' + mapID + '.virtualBoundaries.' + extVirtualBoundaryId + '.deleteVirtualBoundary', 'Delete the virtual Boundary ' + extVirtualBoundaryId,
                        'boolean', 'button', true, false, '');
                }
            });
        }
