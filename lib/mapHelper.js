    ...
    adapter.getChannelsOf('map', function (err, channel) { //check existing spotArea states
        ...
        for (const extSpotAreaId in spotAreaArray) { //create new states
            adapter.getObject(mapChannel + '.' + extSpotAreaId, function (err, spotAreaObj) {
                if (spotAreaObj) {
                    adapter.setStateChanged(mapChannel + '.spotAreas.' + extSpotAreaId + '.spotAreaIsAvailable', true, true);
                    adapter.setStateChanged(spotAreaObj._id + '.spotAreaDeactivationTimestamp', null, true);
                } else { //create spotArea
                    adapter.createChannelNotExists(mapChannel, 'Map ' + mapID);
                    adapter.createChannelNotExists(mapChannel + '.spotAreas.' + extSpotAreaId, 'SpotArea ' + extSpotAreaId);
                    adapter.createObjectNotExists(mapChannel + '.spotAreas.' + extSpotAreaId + '.spotAreaID', 'ID of the SpotArea', 'string', 'text', false, spotAreaArray[extSpotAreaId]['mapSpotAreaID'], '');
                    adapter.createObjectNotExists(mapChannel + '.spotAreas.' + extSpotAreaId + '.spotAreaIsAvailable', 'Is the SpotArea still available?', 'boolean', 'indicator.status', false, true, '');
                    adapter.createObjectNotExists(mapChannel + '.spotAreas.' + extSpotAreaId + '.spotAreaDeactivationTimestamp', 'When was the SpotArea deactivated (null if active)', 'number', 'value.datetime', false, null, '');
                    adapter.createObjectNotExists(mapChannel + '.spotAreas.' + extSpotAreaId + '.spotAreaBoundaries', 'Boundaries of the SpotArea', 'string', 'text', false, '', '');

                    adapter.createObjectNotExists(
                        mapChannel + '.spotAreas.' + extSpotAreaId + '.cleanSpotArea', 'Clean spot area ' + extSpotAreaId,
...
function processSpotAreaInfo(adapter, spotArea) {
    ...
    adapter.getObject(mapChannel + '.spotAreas.' + spotAreaID, function (err, spotAreaObj) {
        ...
        if (spotAreaObj) {
            ...
            adapter.createObjectNotExists(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaID', 'ID of the SpotArea', 'string', 'text', false, spotAreaID, '');
            adapter.createObjectNotExists(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaIsAvailable', 'Is the SpotArea still available?', 'boolean', 'indicator.status', false, true, '');
            adapter.createObjectNotExists(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaDeactivationTimestamp', 'When was the SpotArea deactivated (null if active)', 'number', 'value.datetime', false, null, '');
            adapter.createObjectNotExists(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaName', 'Name of the SpotArea', 'string', 'text', false, mapSpotAreaName, '');
            adapter.createObjectNotExists(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaBoundaries', 'Boundaries of the SpotArea', 'string', 'text', false, spotArea['mapSpotAreaBoundaries'], '');
        }
