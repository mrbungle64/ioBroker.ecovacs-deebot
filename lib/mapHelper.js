                ...
                if (!virtualBoundary) {
                    ...
                    adapter.setStateChanged(virtualBoundaryObj._id + '.virtualBoundaryIsAvailable', true, true, function (err, id, notChanged) {
                        if (!notChanged) {
                            // status has changed
                            const path = id.substring(0, id.lastIndexOf('.'));
                            adapter.setStateConditional(path + '.virtualBoundaryID', virtualBoundary['virtualBoundaryID'], true);
...
function processVirtualBoundaryInfo(adapter, virtualBoundary) {
    ...
    adapter.getObject('map.' + virtualBoundary['mapID'] + '.virtualBoundaries.' + virtualBoundary['mapVirtualBoundaryID'], function (err, virtualBoundaryObj) {
        ...
        if (virtualBoundaryObj) {
            adapter.setStateConditional('map.' + virtualBoundary['mapID'] + '.virtualBoundaries.' + virtualBoundary['mapVirtualBoundaryID'] + '.virtualBoundaryType', virtualBoundary['mapVirtualBoundaryType'], true);
            adapter.getObject('map.' + virtualBoundary['mapID'] + '.virtualBoundaries.' + virtualBoundary['mapVirtualBoundaryID'] + '.virtualBoundaryCoordinates', function (err, obj) {
                if (!obj) {
                    ...
                    adapter.setStateConditional('map.' + virtualBoundary['mapID'] + '.virtualBoundaries.' + virtualBoundary['mapVirtualBoundaryID'] + '.virtualBoundaryCoordinates', virtualBoundary['mapVirtualBoundaryCoordinates'], true);
                }
            });
        } else {
