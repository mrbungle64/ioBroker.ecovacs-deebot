...
function saveVirtualBoundarySet(adapter, mapID) {
    adapter.createChannelNotExists('map.' + mapID + '.savedBoundarySets', 'Saved virtual boundary sets').then(() => {
        ...
        adapter.getChannelsOfAsync('map').then(virtualBoundaryChannels => {
            ...
            const timestamp = Math.floor(Date.now() / 1000);
            const dateTime = adapter.formatDate(new Date(), 'TT.MM.JJJJ SS:mm:ss');
            const savedBoundarySetID = 'map.' + mapID + '.savedBoundarySets.virtualBoundarySet_' + timestamp;
            adapter.setObjectNotExists(
                savedBoundarySetID, {
                    'type': 'state',
                    'common': {
