...
const commands = require('./adapterCommands');
...
function cleanSpotArea(adapter, mapID, mssID) {
    ...
    if (parseInt(mapID) === parseInt(adapter.currentMapID)) {
...
function rerunLastUsedCustomAreaValues(adapter) {
    adapter.getStateAsync('map.lastUsedCustomAreaValues').then(state => {
        if (state && state.val) {
            commands.startCustomAreaCleaning(state.val, adapter.customAreaCleanings);
        }
    });
}
    ...
    if (pattern.test(id)) {
        adapter.getObjectAsync(id).then(obj => {
            if (obj && obj.native && obj.native.area) {
                commands.startCustomAreaCleaning(obj.native.area, adapter.customAreaCleanings);
            }
        });
    }
}
...
function goToPosition(adapter, state) {
    ...
    if (helper.positionValueStringIsValid(goToPositionValues)) {
        ...
        commands.startCustomAreaCleaning(goToAreaValues, 1);
    } else if (state.val !== '') {
