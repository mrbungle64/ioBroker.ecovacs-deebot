    ...
    adapter.getChannelsOf('map', function (err, channel) {
        ...
        for (const r in channel) {
            ...
            if (!mapObj._id.includes('spotAreas') && !mapObj._id.includes('virtualBoundaries') && !mapObj._id.includes('savedCustomAreas')) {
                ...
                const mapChannel = mapObj._id;
                if (!mapArray[mapID]) {
                    // map not existent (anymore)
                    adapter.setStateChanged(mapChannel + '.mapIsAvailable', false, true, function (err, id, notChanged) {
                        if (!notChanged) {
                            // map was available before
                            deactivateSpotAreasForMap(adapter, mapChannel, mapID);
                        }
                    });
                } else {
                    updateMapData(adapter, mapChannel, mapArray[mapID]['mapName']);
                }
        ...
        for (const mapID in mapArray) {
            if (Object.prototype.hasOwnProperty.call(mapArray, mapID)) {
                ...
                adapter.getObject(mapChannel, function (err, mapChannelObj) {
                    if (!mapChannelObj) {
                        createMapData(adapter, mapArray, mapID);
                    } else {
                        updateMapData(adapter, mapChannelObj._id, mapArray[mapID]['mapName']);
                    }
                    delete mapArray[mapID];
                });
            }
...
function createMapData(adapter, mapArray, mapID) {
    const mapChannel = 'map.' + mapID;
...
function updateMapData(adapter, mapChannel, channelName) {
        ...
        for (const spotAreaID in spotAreaArray) {
            if (Object.prototype.hasOwnProperty.call(spotAreaArray, spotAreaID)) {
                ...
                adapter.getObject(spotAreaChannel, function (err, spotAreaObj) {
                    if (!spotAreaObj) {
                        createSpotAreaData(adapter, spotAreaArray, mapID, spotAreaID);
                    } else {
                        updateSpotAreaData(adapter, spotAreaChannel);
                    }
                });
            }
        }
...
function updateSpotAreaData(adapter, spotAreaChannel) {
...
function createSpotAreaData(adapter, spotAreaArray, mapID, spotAreaID) {
    const mapChannel = 'map.' + mapID;
    const spotAreaChannel = mapChannel + '.spotAreas.' + spotAreaID;
    // create spotArea
    adapter.createChannelNotExists(mapChannel, 'Map ' + mapID);
    ...
    adapter.createObjectNotExists(
        ...
        'string', 'text', false, spotAreaArray[spotAreaID]['mapSpotAreaID']);
    adapter.createObjectNotExists(
        ...
        for (const r in channel) {
            ...
            if (virtualBoundaryObj._id.includes(mapID) && virtualBoundaryObj._id.includes('virtualBoundaries.')) {
                ...
                if (!virtualBoundary) {
                    ...
                    adapter.setStateConditional(virtualBoundaryObj._id + '.virtualBoundaryID', virtualBoundary['mapVirtualBoundaryID'], true);
                    adapter.setStateConditional(virtualBoundaryObj._id + '.virtualBoundaryType', virtualBoundary['mapVirtualBoundaryType'], true);
                    adapter.setStateConditional(virtualBoundaryObj._id + '.virtualBoundaryCoordinates', virtualBoundary['mapVirtualBoundaryCoordinates'], true);
                }
            ...
            if (Object.prototype.hasOwnProperty.call(virtualBoundaryArray, extVirtualBoundaryId)) {
                ...
                adapter.getObject(virtualBoundaryChannel, function (err, virtualBoundaryObj) {
                    if (virtualBoundaryObj) {
                        ...
                        createVirtualBoundaryData(adapter, mapChannel, virtualBoundaryChannel, virtualBoundaryArray[extVirtualBoundaryId]);
...
function processVirtualBoundaryInfo(adapter, virtualBoundary) {
    ...
    adapter.getObject(virtualBoundaryChannel, function (err, virtualBoundaryObj) {
        ...
        if (virtualBoundaryObj) {
            ...
            createVirtualBoundaryData(adapter, mapChannel, virtualBoundaryChannel, virtualBoundary);
        }
    });
}

function createVirtualBoundaryData(adapter, mapChannel, virtualBoundaryChannel, virtualBoundary) {
    ...
    adapter.createObjectNotExists(
        ...
        'string', 'text', false, virtualBoundary['mapVirtualBoundaryID']);
    adapter.createObjectNotExists(
        virtualBoundaryChannel + '.virtualBoundaryType', 'Type of the virtualBoundary (Virtual Wall / No Mop Zone)',
        'string', 'text', false, virtualBoundary['mapVirtualBoundaryType']);
    if (virtualBoundary['mapVirtualBoundaryCoordinates']) {
        adapter.createObjectNotExists(
            ...
            'text', false, virtualBoundary['mapVirtualBoundaryCoordinates']);
    }
