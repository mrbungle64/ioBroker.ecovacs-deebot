...
async function saveCurrentSpotAreaValues(adapter, ctx) {
    ...
    if (state && state.val) {
        await ctx.adapterProxy.createChannelNotExists('map.savedSpotAreas', 'Saved areas');
        ...
        ctx.adapter.log.info('Saving current spot area values: ' + state.val + ' on map ' + ctx.currentMapID);
        adapter.setObjectNotExists(
            savedAreaID, {
                type: 'state',
                common: {
                    name: 'myAreaName (mapID ' + ctx.currentMapID + ', spotArea ' + state.val + ')',
                    type: 'boolean',
                    role: 'button',
                    read: true,
                    write: true,
...
async function saveLastUsedCustomAreaValues(adapter, ctx) {
    ctx.adapterProxy.getStateAsync('map.lastUsedCustomAreaValues').then(state => {
        if (state && state.val) {
            ctx.adapterProxy.createChannelNotExists('map.savedCustomAreas', 'Saved areas').then(() => {
                const timestamp = helper.getUnixTimestamp();
                ...
                ctx.adapterProxy.getObjectAsync('map.lastUsedCustomAreaValues').then(obj => {
                    if (obj) {
