...
async function saveVirtualBoundary(adapter, mapID, mssID) {
    ...
    const obj = await adapter.getObjectAsync(getCurrentMapChannel(adapter, mapID) + '.virtualBoundaries.' + mssID);
...
async function deleteVirtualBoundary(adapter, mapID, mssID) {
    const objID = getCurrentMapChannel(adapter, mapID) + '.virtualBoundaries.' + mssID;
