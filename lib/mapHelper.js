...
function processSpotAreaInfo(adapter, spotArea) {
    ...
    adapter.getObject('map.' + spotArea['mapID'] + '.spotAreas.' + spotArea['mapSpotAreaID'], function (err, spotAreaObj) {
        ...
        if (spotAreaObj) {
            ...
            let id = 'map.' + spotArea['mapID'] + '.spotAreas.' + spotArea['mapSpotAreaID'] + '.spotAreaName';
            adapter.getObject(id, function (err, obj) {
                if (!obj) {
                    ...
                    adapter.createObjectNotExists('map.' + spotArea['mapID'] + '.spotAreas.' + spotArea['mapSpotAreaID'] + '.spotAreaName', 'Name of the SpotArea', 'string', 'text', false, mapSpotAreaName, '');
                } else {
                    adapter.setStateChanged(id, mapSpotAreaName, true);
                    if (spotAreaSync === 'fullSynchronization') {
                        // control channel
                        id = 'control.spotArea_' + spotArea['mapSpotAreaID'];
                        adapter.getObject(id, function (err, obj) {
        ...
        if ((spotAreaSync === 'onlyCreate') || (spotAreaSync === 'fullSynchronization')) {
            adapter.createObjectNotExists(
                'control.spotArea_' + spotArea['mapSpotAreaID'], mapSpotAreaName,
                'boolean', 'button', true, false, '');
        }
    });
}
