...
function saveVirtualBoundary(adapter, mapID, mssID) {
    adapter.createChannelNotExists('map.savedBoundaries', 'Saved virtual boundaries in the map for de-/activation').then(() => {
        ...
        adapter.getStateAsync('map.' + mapID + '.virtualBoundaries.' + mssID + '.virtualBoundaryType').then(state => {
            if (state && state.val) {
                const savedBoundaryType = state.val;
                adapter.getStateAsync('map.' + mapID + '.virtualBoundaries.' + mssID + '.virtualBoundaryCoordinates').then(state => {
                    if (state && state.val) {
                        adapter.createChannelNotExists('map.savedBoundaries', 'Saved virtual boundaries in the map for de-/activation').then(() => {
                            const timestamp = Math.floor(Date.now() / 1000);
...
function addVirtualBoundary(adapter, stateId) {
    adapter.getObjectAsync(stateId).then(obj => {
        if (obj) {
            if ((obj.native) && (obj.native.dateTime) && (obj.native.currentMapID)) {
                adapter.log.info('Add virtual boundary on map ' + obj.native.currentMapID + ' with type ' + obj.native.boundaryType);
