...
function rerunLastUsedCustomAreaValues(adapter) {
    adapter.getStateAsync('map.lastUsedCustomAreaValues').then(state => {
        if (state && state.val) {
            commands.startCustomAreaCleaning(adapter, state.val, adapter.customAreaCleanings);
        }
    });
}
    ...
    if (pattern.test(id)) {
        adapter.getObjectAsync(id).then(obj => {
            if (obj && obj.native && obj.native.area) {
                commands.startCustomAreaCleaning(adapter, obj.native.area, adapter.customAreaCleanings);
            }
        });
    }
}
