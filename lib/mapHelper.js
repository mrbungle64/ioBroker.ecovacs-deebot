                ...
                if (!map) {
                    ...
                    adapter.setStateChanged(mapObj._id + '.mapIsAvailable', false, true, function (err, id, notChanged) {
                        if (!notChanged) {
                            ...
                            adapter.setStateChanged(mapObj._id + '.mapDeactivationTimestamp', timestamp, true);
                        }
                    });
                } else {
...
function processSpotAreas(adapter, spotAreas) {
    ...
    adapter.getChannelsOf('map', function (err, channel) {
        ...
        for (const r in channel) {
            ...
            if (!spotAreaObj._id.includes('spotAreas.') || !spotAreaObj._id.includes(mapID)) {
                // only process subchannels of spotAreas of specified map
            } else {
                ...
                if (!spotArea) {
                    ...
                    adapter.setStateChanged(spotAreaObj._id + '.spotAreaIsAvailable', false, true, function (err, id, notChanged) {
                        if (!notChanged) {
                            ...
                            const timestamp = Math.floor(Date.now() / 1000);
                            adapter.setStateChanged(spotAreaObj._id + '.spotAreaDeactivationTimestamp', timestamp, true);
...
function processVirtualBoundaries(adapter, virtualBoundaries) {
    ...
    adapter.getChannelsOf('map', function (err, channel) { //check existing virtualBoundary states
        ...
        for (const r in channel) {
            ...
            if (!virtualBoundaryObj._id.includes('virtualBoundaries') || !virtualBoundaryObj._id.includes(mapID)) {
                // only process subchannels of virtualBoundaries.availableBoundaries of specified map
            } else {
        ...
        for (const extVirtualBoundaryId in virtualBoundaryArray) {
            ...
            adapter.getObject(virtualBoundaryChannel, function (err, virtualBoundaryObj) {
                if (virtualBoundaryObj) {
                    ...
                    adapter.setStateChanged(virtualBoundaryChannel + '.virtualBoundaryType', virtualBoundaryArray[extVirtualBoundaryId]['mapVirtualBoundaryType'], true);
                } else {
