...
async function saveGoToPositionValues(adapter, goToPositionValues, savedAreaID = '', name = '') {
    await adapter.createChannelNotExists('control.extended.savedGoToPositionValues', 'Saved go to position values');
    if (savedAreaID === '') {
        const timestamp = helper.getUnixTimestamp();
        savedAreaID = 'control.extended.savedGoToPositionValues.goToPosition_' + timestamp;
        adapter.log.info('Saving go to position values: ' + goToPositionValues + ' on map ' + adapter.currentMapID);
    }
    if (name === '') {
        name = 'myGoToPosition (mapID ' + adapter.currentMapID + ', position ' + goToPositionValues + ')';
    }
    const dateTime = adapter.getCurrentDateAndTimeFormatted();
    adapter.setObjectNotExists(
        savedAreaID, {
...
async function saveVirtualBoundary(adapter, mapID, mapSpotAreaID) {
    ...
    adapter.log.info('Saving virtual boundary: ' + mapSpotAreaID + ' on map ' + mapID);
    const obj = await adapter.getObjectAsync('map.' + mapID + '.virtualBoundaries.' + mapSpotAreaID);
...
async function deleteVirtualBoundary(adapter, mapID, mapSpotAreaID) {
    ...
    if (obj) {
        ...
        if (state && state.val) {
            ...
            adapter.log.info('Delete virtual boundary on server: ' + mapSpotAreaID + ' on map ' + mapID + ' with type ' + type);
            adapter.commandQueue.run('DeleteVirtualBoundary', mapID.toString(), mapSpotAreaID, type);
        }
    }
    adapter.vacbot.run('GetMaps');
}
