...
async function processMapData(adapter, mapData) {
    for (const i in mapData['maps']) {
        if (Object.prototype.hasOwnProperty.call(mapData['maps'], i)) {
            const mapChannel = await getCurrentMapChannel(adapter, mapData['maps'][i]['mapID']);
...
async function createSpotAreaObjectsNotExists(adapter, mapID, spotAreaID) {
    const spotAreaChannel = await getCurrentMapChannel(adapter, mapID) + '.spotAreas.' + spotAreaID;
    // create spotArea
    ...
    await adapter.createObjectNotExists(
        ...
        'number', 'value.datetime', false, 0);
    await adapter.createObjectNotExists(
        spotAreaChannel + '.lastTimeLeavedTimestamp', 'Timestamp for internal use',
        'number', 'value.datetime', false, 0);
    await adapter.createObjectNotExists(
...
async function processVirtualBoundaryInfo(adapter, virtualBoundaryArray) {
    const mapChannel = await getCurrentMapChannel(adapter, virtualBoundaryArray['mapID']);
...
async function getCurrentMapChannel(adapter, mapID) {
    const fixedMapID = adapter.getConfigValue('workaround.map.fixedMapID');
    if (fixedMapID !== '') {
        mapID = fixedMapID;
    }
    return 'map.' + mapID;
