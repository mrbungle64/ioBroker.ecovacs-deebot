...
function retrieveSpotAreaNameFromAPI(adapter, mapSpotAreaName) {
    ...
    if (mapSpotAreaName) {
        ...
        // If 'spotAreaKeepModifiedNames' is enabled,
        // the spot area name is not retrieved via the API
        retrieveSpotAreaNameFromAPI = false;
        if (keepModifiedName !== 'enable') {
            // If 'spotAreaKeepModifiedNames' is 'disabled' or 'pre-selection',
            // the spot area name is retrieved via the API
            // but Labels and custom names are not supported by older models
