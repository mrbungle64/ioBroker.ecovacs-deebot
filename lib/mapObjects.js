...
async function processMaps(adapter, mapData) {
    ...
    const mapArray = await processMapData(adapter, mapData);
    await processMapChannels(adapter, mapArray);
}
        ...
        if (spotAreaObj) {
            updateSpotAreaStates(adapter, mapChannel + '.spotAreas.' + spotAreaID, spotArea).then(() => {
                adapter.setStateConditionalAsync(mapChannel + '.spotAreas.' + spotAreaID + '.spotAreaName', mapSpotAreaName, true).then(() => {
                    // control channel
                    if (spotAreaSync === 'fullSynchronization') {
                        adapter.getObjectAsync(mapChannel + '.spotAreas.' + spotAreaID).then(obj => {
                            if (obj && obj.common) {
                                obj.common.name = 'Spot area ' + spotAreaID + ' (' + mapSpotAreaName + ')';
                                adapter.extendObject(mapChannel + '.spotAreas.' + spotAreaID, obj, function (err) {
                                    if (err) {
...
async function updateVirtualBoundaryStates(adapter, virtualBoundaryChannel, virtualBoundary) {
    ...
    await adapter.extendObjectAsync(virtualBoundaryChannel, {
        native: {
