...
async function processSpotAreas(adapter, ctx, spotAreas) {
    ...
    for (const spotAreaID in spotAreaArray) {
        if (Object.prototype.hasOwnProperty.call(spotAreaArray, spotAreaID)) {
            ...
            const spotAreaSync = ctx.adapter.getConfigValue('feature.control.spotAreaSync', ctx.deviceId);
...
async function processSpotAreaInfo(adapter, ctx, spotArea) {
    ...
    if (spotAreaObj) {
        ...
        if (ctx.adapter.getConfigValue('feature.control.spotAreaSync', ctx.deviceId) === 'fullSynchronization') {
            // control channel
...
function retrieveSpotAreaNameFromAPI(adapter, ctx, mapSpotAreaName) {
    ...
    if (mapSpotAreaName !== '') {
        const keepModifiedName = ctx.adapter.getConfigValue('feature.control.spotAreaKeepModifiedNames', ctx.deviceId);
