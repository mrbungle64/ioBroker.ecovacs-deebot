...
async function createMapObjectsNotExists(adapter, mapArray) {
    ...
    await adapter.deleteObjectIfExists(mapChannel + '.mapIsAvailable');
    await adapter.createObjectNotExists(
        mapChannel + '.isAvailable', 'Is the map still available?',
        'boolean', 'indicator.status', false, true);
    await adapter.deleteObjectIfExists(mapChannel + '.mapDeactivationTimestamp');
    await adapter.createObjectNotExists(
        mapChannel + '.deactivationTimestamp', 'When was the map deactivated (null if active)',
...
async function updateMapStates(adapter, mapChannel, isAvailable) {
    if (isAvailable) {
        await adapter.setStateConditionalAsync(mapChannel + '.deactivationTimestamp', null, true);
    } else {
        await adapter.deleteObjectIfExists(mapChannel + '.mapIsAvailable');
        await adapter.createObjectNotExists(
            mapChannel + '.isAvailable', 'Is the map still available?',
        ...
        await adapter.setStateConditionalAsync(mapChannel + '.deactivationTimestamp', timestamp, true);
    }
    await adapter.setStateConditionalAsync(mapChannel + '.isAvailable', isAvailable, true);
}
            ...
            if (mapHelper.isSpotAreasChannel(spotAreaObj._id) && spotAreaObj._id.includes('.' + mapID + '.')) {
                ...
                if (!spotAreaArray[spotAreaID]) {
                    ...
                    if (state && (state.val === true)) {
                        ...
                        await adapter.deleteObjectIfExists(spotAreaObj._id + '.spotAreaIsAvailable');
                        await adapter.createObjectNotExists(
                            spotAreaObj._id + '.isAvailable', 'Is the spot area still available?',
                            'boolean', 'indicator.status', false, true);
                        await adapter.deleteObjectIfExists(spotAreaObj._id + '.spotAreaDeactivationTimestamp');
                        await adapter.createObjectNotExists(
                            spotAreaObj._id + '.deactivationTimestamp', 'When was the spot area deactivated (null if active)',
                            'number', 'value.datetime', false, null);
                        await adapter.setStateConditionalAsync(spotAreaObj._id + '.isAvailable', false, true);
                        await adapter.setStateConditionalAsync(spotAreaObj._id + '.deactivationTimestamp', timestamp, true);
...
async function createSpotAreaObjectsNotExists(adapter, mapID, spotAreaID) {
    ...
    await adapter.deleteObjectIfExists(spotAreaChannel + '.spotAreaIsAvailable');
    await adapter.createObjectNotExists(
        spotAreaChannel + '.isAvailable', 'Is the spot area still available?',
        'boolean', 'indicator.status', false, true);
    await adapter.deleteObjectIfExists(spotAreaChannel + '.spotAreaDeactivationTimestamp');
    await adapter.createObjectNotExists(
        spotAreaChannel + '.deactivationTimestamp', 'When was the spot area deactivated (null if active)',
        'number', 'value.datetime', false, null);
    await adapter.createObjectNotExists(
...
async function updateSpotAreaStates(adapter, spotAreaChannel, spotAreaArray) {
    await adapter.setStateConditionalAsync(spotAreaChannel + '.isAvailable', true, true);
    await adapter.setStateConditionalAsync(spotAreaChannel + '.deactivationTimestamp', null, true);
