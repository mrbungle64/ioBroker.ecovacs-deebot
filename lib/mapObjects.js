...
async function createMapObjectsNotExists(adapter, mapArray) {
    ...
    const mapChannel = getCurrentMapChannel(adapter, mapID);
...
async function createSpotAreaObjectsNotExists(adapter, mapID, spotAreaID) {
    const spotAreaChannel = getCurrentMapChannel(adapter, mapID) + '.spotAreas.' + spotAreaID;
    // create spotArea
...
function getCurrentMapChannel(adapter, mapID) {
