...
async function processMaps(adapter, mapData) {
    adapter.spotAreaSequenceNumbers = [];
...
async function updateSpotAreaStates(adapter, spotAreaChannel, spotAreaArray) {
    ...
    if (spotAreaArray['mapSpotAreaSequenceNumber'] >= 0) {
        ...
        if (adapter.currentMapID == spotAreaArray['mapID']) {
            const sequenceNumberObj = {
                mapSpotAreaID: spotAreaArray['mapSpotAreaID'],
                sequenceNumber: spotAreaArray['mapSpotAreaSequenceNumber']
            };
            adapter.spotAreaSequenceNumbers.push(sequenceNumberObj);
        }
    }
