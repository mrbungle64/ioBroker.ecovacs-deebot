...
async function createSpotAreaObjectsNotExists(adapter, mapID, spotAreaID) {
    ...
    await adapter.createObjectNotExists(
        ...
        'string', 'text', false, '');
    await adapter.createObjectNotExists(
        spotAreaChannel + '.spotAreaSequenceNumber', 'Sequence index for the spot area',
        'number', 'value', false, null);
    await adapter.createObjectNotExists(
...
async function updateSpotAreaStates(adapter, spotAreaChannel, spotAreaArray) {
    ...
    if (spotAreaArray['index']) {
        await adapter.setStateConditionalAsync(spotAreaChannel + '.spotAreaSequenceNumber', spotAreaArray['index'], true);
    }
