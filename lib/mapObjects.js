...
function processMaps(adapter, mapData) {
    ...
    adapter.log.silly('[silly][processMaps] Processing map data');
    for (const i in mapData['maps']) {
        if (Object.prototype.hasOwnProperty.call(mapData['maps'], i)) {
            ...
            if (adapter.getModel().isSupportedFeature('map.spotAreas')) {
                adapter.intervalQueue.add('GetSpotAreas', mapID);
                adapter.log.silly('[silly][processMaps] Run GetSpotAreas cmd for mapID: ' + mapID);
            }
            if (adapter.getModel().isSupportedFeature('map.virtualBoundaries')) {
                adapter.intervalQueue.add('GetVirtualBoundaries', mapID);
            }
        }
    }
...
function processSpotAreas(adapter, spotAreas) {
    ...
    adapter.log.silly('[silly][processSpotAreas] Processing spot areas for mapID ' + mapID);
    if (!mapID) {
    ...
    if (spotAreas['mapSpotAreas'].length) {
        adapter.createChannelNotExists(mapChannel + '.spotAreas', 'SpotAreas of the map');
    }
    ...
    adapter.getChannelsOf('map', function (err, channel) {
        ...
        for (const spotAreaID in spotAreaArray) {
            if (Object.prototype.hasOwnProperty.call(spotAreaArray, spotAreaID)) {
                ...
                adapter.intervalQueue.add('GetSpotAreaInfo', mapID, spotAreaID);
                adapter.log.silly('[silly][processSpotAreas] Run GetSpotAreaInfo cmd for mapID ' + mapID + ' and spotAreaID ' + spotAreaID);
            }
        }
    });
}
...
function processVirtualBoundaries(adapter, virtualBoundaries) {
    ...
    adapter.getChannelsOf('map', function (err, channel) {
        ...
        for (const r in channel) {
            ...
            if (mapHelper.isVirtualBoundariesChannel(virtualBoundaryObj._id)) {
        ...
        for (const extVirtualBoundaryId in virtualBoundaryArray) {
            if (Object.prototype.hasOwnProperty.call(virtualBoundaryArray, extVirtualBoundaryId)) {
                ...
                adapter.intervalQueue.add('GetVirtualBoundaryInfo', mapID, virtualBoundaryArray[extVirtualBoundaryId]['mapVirtualBoundaryID'], virtualBoundaryArray[extVirtualBoundaryId]['mapVirtualBoundaryType']);
            }
        }
    });
}
