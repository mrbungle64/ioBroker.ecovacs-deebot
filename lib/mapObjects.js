...
async function processVirtualBoundaries(adapter, virtualBoundaries) {
    ...
    const processVirtualBoundaries = new Promise(() => {
    ...
    processVirtualBoundaries.then(() => {
        processVirtualBoundaryChannels(adapter, virtualBoundaryArray, mapID);
    });
}
...
async function processVirtualBoundaryInfo(adapter, virtualBoundaryArray) {
    ...
    adapter.getObjectAsync(virtualBoundaryChannel).then((virtualBoundaryObj) => {
        createOrUpdateVirtualBoundaryObjects(adapter, mapChannel, virtualBoundaryChannel, virtualBoundaryObj, virtualBoundaryArray);
    });
}

async function createOrUpdateVirtualBoundaryObjects(adapter, mapChannel, virtualBoundaryChannel, virtualBoundaryObj, virtualBoundaryArray) {
    if (virtualBoundaryObj) {
        await updateVirtualBoundaryStates(adapter, virtualBoundaryChannel, virtualBoundaryArray);
    } else {
        await createVirtualBoundaryObjects(adapter, mapChannel, virtualBoundaryArray['mapVirtualBoundaryID'], virtualBoundaryArray['mapVirtualBoundaryType']);
    }
}

async function createVirtualBoundaryObjects(adapter, virtualBoundaryChannel, mapVirtualBoundaryID, mapVirtualBoundaryType) {
    await adapter.createChannelNotExists(virtualBoundaryChannel, 'virtualBoundary ' + mapVirtualBoundaryID);
    await adapter.createObjectNotExists(
...
async function processVirtualBoundaryChannels(adapter, virtualBoundaryArray, mapID) {
    adapter.getChannelsOf('map', function (err, channel) {
