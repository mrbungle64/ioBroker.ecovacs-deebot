...
async function processMapData(adapter, mapData) {
    for (const i in mapData['maps']) {
        if (Object.prototype.hasOwnProperty.call(mapData['maps'], i)) {
            const mapChannel = await mapHelper.getCurrentMapChannel(adapter, mapData['maps'][i]['mapID']);
...
async function createMapObjectsNotExists(adapter, mapArray) {
    ...
    const mapChannel = await mapHelper.getCurrentMapChannel(adapter, mapID);
...
async function createSpotAreaObjectsNotExists(adapter, mapID, spotAreaID) {
    const spotAreaChannel = await mapHelper.getCurrentMapChannel(adapter, mapID) + '.spotAreas.' + spotAreaID;
    // create spotArea
...
async function processVirtualBoundaryInfo(adapter, virtualBoundaryArray) {
    const mapChannel = await mapHelper.getCurrentMapChannel(adapter, virtualBoundaryArray['mapID']);
