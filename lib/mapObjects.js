...
async function updateSpotAreaStates(adapter, spotAreaChannel, spotAreaArray) {
    ...
    if (spotAreaArray['mapSpotAreaCleanSet']) {
        await adapter.createObjectNotExists(
            spotAreaChannel + '.cleaningPreference', 'Cleaning preference values for this spot area (ready-only)',
            'json', 'value', false, null, '');
...
async function setCleaningPreferenceValues(adapter, spotAreaChannel, cleanSetObj) {
    await adapter.setStateConditionalAsync(spotAreaChannel + '.cleaningPreference', JSON.stringify(cleanSetObj), true);
    const obj = await adapter.getObjectAsync(spotAreaChannel + '.cleaningPreference');
    if (obj) {
        obj.native = {
        ...
        await adapter.extendObjectAsync(spotAreaChannel + '.cleaningPreference', obj);
    }
}
