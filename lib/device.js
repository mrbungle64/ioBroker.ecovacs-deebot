...
class Device {
     ...
     * and handle the strange behavior of the Deebot 900/901
     * @param {number} batteryLevel
     */
    setBatteryLevel(batteryLevel) {
        if ((this.ctx.getModel().isModel900Series()) && this.batteryLevel) {
            if (this.isCharging() && (batteryLevel > this.batteryLevel)) {
                this.batteryLevel = batteryLevel;
            } else if (this.isNotCharging() && (batteryLevel < this.batteryLevel)) {
                this.batteryLevel = batteryLevel;
            } else {
                this.ctx.adapter.log.debug('Ignoring battery level value: ' + batteryLevel + ' (current value: ' + this.batteryLevel + ')');
            }
        } else {
            this.batteryLevel = batteryLevel;
        }
    ...
    useV2commands() {
        const configValue = this.ctx.adapter.getConfigValue('feature.control.v2commands');
        if (configValue === '') {
    ...
    useNativeGoToPosition() {
        const configValue = this.ctx.adapter.getConfigValue('feature.control.nativeGoToPosition');
        if (configValue === '') {
