...
class Device {
    ...
    setBattery(newValue) {
        // There's a strange behavior of the battery value on Deebot 900/901 (class 'ls1ok3')
        // It's very likely that this is a firmware bug
        if ((this.adapter.getModel().getClass() === 'ls1ok3') && this.battery) {
            if (this.isCharging() && (newValue > this.battery)) {
                this.battery = newValue;
            } else if (this.isNotCharging() && (newValue < this.battery)) {
                this.battery = newValue;
            } else {
                this.adapter.log.debug('Ignoring battery level value: ' + newValue + ' (current value: ' + this.battery + ')');
            }
        } else {
            this.battery = newValue;
        }
