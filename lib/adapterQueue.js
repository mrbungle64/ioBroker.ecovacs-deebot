    ...
    startNextItemFromQueue(runAll = false) {
        ...
        if (queued) {
            ...
            if (queued.arg1 && queued.arg2) {
                ...
                this.adapter.log.debug('[' + this.name + '] ' + queued.cmd + ', ' + queued.arg1 + ', ' + queued.arg2);
            } else if (queued.arg1) {
                this.adapter.vacbot.run(queued.cmd, queued.arg1);
                this.adapter.log.debug('[' + this.name + '] ' + queued.cmd + ', ' + queued.arg1);
            } else {
                this.adapter.vacbot.run(queued.cmd);
                this.adapter.log.debug('[' + this.name + '] ' + queued.cmd);
            }
            this.adapter.log.silly('[' + this.name + '] Removed ' + queued.cmd + ' from ' + this.name + ' (' + this.entries.length + ' left)');
        }
