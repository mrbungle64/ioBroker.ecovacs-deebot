...
class Queue {
    ...
    addGetCleanLogs() {
        const model = new Model(this.adapter.vacbot.deviceClass, this.adapter.config);
        if (model.isSupportedFeature('cleaninglog.channel')) {
            this.add('GetCleanSum');
            if (this.adapter.vacbot.useMqtt && (!this.adapter.vacbot.is950type())) {
                this.add('GetLogApiCleanLogs');
            } else {
                if (this.adapter.config['workaround.lastCleaningAPICall']) {
                    this.add('GetCleanLogsWithoutLastInfo');
                    this.add('GetLastCleanLogInfo');
                } else {
