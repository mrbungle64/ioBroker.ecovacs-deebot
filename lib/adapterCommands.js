...
async function handleStateChange(adapter, ctx, id, state) {
    ...
    if (channelName !== 'history') {
        ...
        if ((stateName === 'error') && (ctx.connectionFailed) && (!adapter.authFailed)) {
            if ((!ctx.retrypauseTimeout) && (ctx.retries <= MAX_RETRIES)) {
                ctx.retrypauseTimeout = setTimeout(() => {
                    adapter.reconnect();
                }, RETRY_PAUSE);
            }
        }
    ...
    if (channelName === 'info') {
        if (stateName === 'currentSchedule_refresh') {
            ctx.vacbot.run(handleV2commands(adapter, 'GetSchedule'));
    ...
    if (channelName === 'control') {
            ...
            switch (stateName) {
                ...
                case 'spotArea': {
                    ...
                    if (ctx.vacbot.is950type() && (ctx.spotAreaCleanings === 2)) {
                        startSpotAreaCleaning(adapter, state.val, ctx.spotAreaCleanings);
                ...
                case 'customArea': {
                    ...
                    if (helper.areaValueStringWithCleaningsIsValid(customAreaValues)) {
                        ...
                        startCustomAreaCleaning(adapter, customAreaValues, ctx.customAreaCleanings);
                    } else {
        ...
        switch (stateName) {
            case 'clean':
                ...
                ctx.vacbot.run(handleV2commands(adapter, stateName));
                adapter.clearGoToPosition();
                break;
            case 'edge':
            case 'spot':
...
function rerunLastUsedCustomAreaValues(adapter) {
    ctx.adapterProxy.getStateAsync('map.lastUsedCustomAreaValues').then(state => {
        if (state && state.val) {
            startCustomAreaCleaning(adapter, state.val, ctx.customAreaCleanings);
        }
    }).catch(e => {
...
function cleanSavedCustomArea(adapter, id) {
    ...
    if (pattern.test(id)) {
        ctx.adapterProxy.getObjectAsync(id).then(obj => {
            if (obj && obj.native && obj.native.area) {
                startCustomAreaCleaning(adapter, obj.native.area, ctx.customAreaCleanings);
            }
        }).catch(e => {
