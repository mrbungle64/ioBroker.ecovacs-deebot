        ...
        if (state.val !== '') {
            switch (stateName) {
                case 'spotArea_silentApproach': {
                    ...
                    const spotAreaString = await getSortedSpotAreasBySequenceNumbers(mapSpotAreas.val);
...
async function getSortedSpotAreasBySequenceNumbers(adapter, unorderedSpotAreaString) {
    const mapSpotAreasArray = unorderedSpotAreaString.split(',');
    adapter.spotAreaSequenceNumbers.sort((a, b) => {
        return a.sequenceNumber - b.sequenceNumber;
    });
    const orderedSpotAreaArray = [];
    adapter.spotAreaSequenceNumbers.forEach((item) => {
        if (mapSpotAreasArray.includes(item.mapSpotAreaID.toString())) {
            orderedSpotAreaArray.push(item.mapSpotAreaID);
        }
    });
    return orderedSpotAreaArray.toString();
}
