    ...
    if ((channelName === 'control') && (subChannelName === 'extended')) {
        switch (stateName) {
            ...
            case 'autoBoostSuction': {
                adapter.vacbot.run('SetCarpetPressure', Number(state.val));
                adapter.log.info('Set autoBoostSuction: ' + state.val);
                break;
            }
            case 'cleaningClothReminder': {
                adapter.vacbot.run('SetDusterRemind', Number(state.val));
                adapter.log.info('Set cleaningClothReminder: ' + state.val);
                break;
            }
            case 'trueDetect': {
...
/**
 * If the feature.control.v2commands is set to true, then the command is appended with _V2
 * @param {Object} adapter - The adapter object
 * @param {string} command - The command to send to the device
 * @returns The command name with the "_V2" suffix if the feature.control.v2commands is set to true
 */
