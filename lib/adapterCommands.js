...
async function handleStateChange(adapter, id, state) {
    ...
    if ((channelName === 'control') && (subChannelName === 'genericCommand')) {
        if (stateName === 'run') {
            ...
            await adapter.setStateAsync('control.extended.genericCommand.responsePayload', '', false);
            ...
            if (payload) {
                ...
                await adapter.setStateAsync('control.extended.genericCommand.payload', payload, true);
            } else {
                adapter.vacbot.run('Generic', command);
                await adapter.setStateAsync('control.extended.genericCommand.payload', '', true);
            }
...
function runSetCleanSpeed(adapter, value) {
    ...
    if (adapter.getModel().getModelType() === 'airbot') {
