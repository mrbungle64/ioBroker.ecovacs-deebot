...
async function handleStateChange(adapter, id, state) {
    ...
    if (channelName === 'map') {
        ...
        const path = id.split('.');
        const mapChannelName = path[3];
        let mapID = mapChannelName;
        const mapIdState = await adapter.getStateAsync('map.' + mapChannelName + '.mapID');
        if (mapIdState && mapIdState.val) {
            mapID = mapIdState.val;
        }
        const mssID = path[5];
