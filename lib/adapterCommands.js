...
async function handleStateChange(adapter, id, state) {
    ...
    if ((channelName === 'control') && (subChannelName === 'genericCommand')) {
        if (stateName === 'run') {
            const commandState = await adapter.getStateAsync('control.extended.genericCommand.command');
            const payloadState = await adapter.getStateAsync('control.extended.genericCommand.payload');
            ...
            adapter.log.info('Run generic cmd: ' + command);
            if (payload) {
                adapter.vacbot.run('Generic', command, payload);
                adapter.log.info('Payload: ' + JSON.stringify(payload));
                await adapter.setStateAsync('control.extended.genericCommand.payload', JSON.stringify(payload), true);
            } else {
                adapter.vacbot.run('Generic', command);
            }
            return;
        }
    }
    ...
    if (channelName === 'cleaninglog') {
        if (stateName === 'requestCleaningLog') {
            adapter.log.info('Cleaning log was requested (manually triggered)');
            adapter.vacbot.run('GetCleanLogs');
            return;
        }
    }
    ...
    if (channelName === 'map') {
        ...
        if (mapSpotAreaPattern.test(id)) {
            ...
            if ((Number(mapID) === Number(adapter.currentMapID)) &&
                (Number(mapSpotAreaID) === Number(adapter.currentSpotAreaID))) {
    ...
    if ((channelName === 'control') && (subChannelName === 'linkedPurification')) {
        switch (stateName) {
            case 'selfLinkedPurification': {
                ...
                adapter.vacbot.run('SetAutonomousClean', value);
                adapter.log.info('Set linkedPurification: ' + state.val);
                break;
            }
            case 'linkedPurificationAQ': {
                ...
                adapter.vacbot.run('SetAirbotAutoModel', enable, aqEnd, aqStart);
                adapter.log.info('Set linkedPurificationAQ: ' + state.val);
                break;
            }
            default:
                adapter.log.warn('Unhandled control.linkedPurification state: ' + stateName + ' - ' + id);
                return;
        }
    }
    ...
    if ((channelName === 'control') && (subChannelName === 'airPurifierModules')) {
        switch (stateName) {
            case 'uvSanitization': {
                ...
                adapter.vacbot.run('SetUVCleaner', enable);
                adapter.log.info('Set uvSanitization: ' + enable);
                break;
            }
            case 'airFreshening': {
                ...
                adapter.vacbot.run('SetFreshenerLevel', level, enable);
                adapter.log.info('Set airFreshening: ' + state.val);
                break;
            }
            case 'humidification': {
                let level = Number(state.val);
                ...
                adapter.vacbot.run('SetHumidifierLevel', level, enable);
                adapter.log.info('Set humidification: ' + state.val);
                break;
            }
            default:
                adapter.log.warn('Unhandled control.airRefreshingSystem state: ' + stateName + ' - ' + id);
                return;
        }
    }
        ...
        switch (stateName) {
            ...
            case 'atmoVolume': {
                ...
                if ((volume >= 0) && (volume <= 16)) {
                    adapter.vacbot.run('SetAtmoVolume', volume);
                    adapter.log.info('Set atmo volume: ' + volume);
                }
                break;
            }
            case 'atmoLight': {
                ...
                if ((brightness >= 0) && (brightness <= 4)) {
                    adapter.vacbot.run('SetAtmoLight', brightness);
                    adapter.log.info('Set atmo light/brightness: ' + brightness);
                }
                break;
            }
            case 'bluetoothSpeaker': {
                const enable = Number(state.val);
                adapter.vacbot.run('SetBlueSpeaker', enable);
                adapter.log.info('Set bluetoothSpeaker: ' + state.val);
                break;
            }
            case 'microphone': {
                const enable = Number(state.val);
                adapter.vacbot.run('SetMic', enable);
                adapter.log.info('Set microphone: ' + state.val);
                break;
            }
            case 'voiceReport': {
                const enable = Number(state.val);
                adapter.vacbot.run('SetVoiceSimple', enable);
                adapter.log.info('Set voiceReport: ' + state.val);
                break;
            }
            case 'advancedMode': {
            ...
            case 'airDryingDuration': {
                ...
                adapter.vacbot.run('SetDryingDuration', duration);
                adapter.log.info('Set air drying duration: ' + duration + ' min.');
                break;
            }
            case 'cleaningMode': {
                const mode = state.val;
                adapter.vacbot.run('SetWorkMode', mode);
                adapter.log.info('Set cleaning mode: ' + mode);
                break;
            }
            case 'carpetCleaningStrategy': {
                const mode = state.val;
                adapter.vacbot.run('SetCarpetInfo', mode);
                adapter.log.info('Set Carpet cleaning strategy: ' + mode);
                break;
            }
            ...
            case 'moppingMode': {
                adapter.vacbot.run('SetSweepMode', state.val);
                adapter.log.info(`Set sweep mode (mopping mode): ${state.val}`);
                break;
            }
            case 'scrubbingPattern': {
                adapter.vacbot.run('SetWaterLevel', adapter.waterLevel, state.val);
                adapter.log.info(`Set scrubbing pattern: ${state.val}`);
                setTimeout(() => {
            ...
            default:
                adapter.log.warn('Unhandled control.extended state: ' + stateName + ' - ' + id);
                return;
        }
    }
    ...
    if (channelName === 'consumable') {
        ...
        switch (stateName) {
            ...
            case 'airFreshener_reset':
                adapter.log.debug('Reset air freshener to 100%');
                adapter.commandQueue.add('ResetLifeSpan', 'air_freshener');
                break;
            default:
    ...
    if (channelName === 'control') {
            ...
            switch (stateName) {
                case 'spotArea_silentApproach': {
                    ...
                    if (spotAreaString !== '') {
                        ...
                        if (firstSpotArea === Number(adapter.currentSpotAreaID)) {
                ...
                case 'spotArea': {
                    ...
                    if (adapter.vacbot.is950type() && (adapter.spotAreaCleanings === 2)) {
                        ...
                        if (adapter.spotAreaCleanings > 1) {
                            ...
                            adapter.cleaningQueue.createMultipleCleaningsForSpotArea(channelName, stateName, state.val);
                        }
                    }
                    adapter.clearGoToPosition();
                    break;
        ...
        switch (stateName) {
            ...
            case 'spotPurification':
                ...
                adapter.vacbot.run('spotPurification', state.val);
                break;
            case 'resume':
            case 'playSound':
                adapter.log.info('Run: ' + stateName);
...
function runSetCleanSpeed(adapter, value) {
    ...
    if (adapter.getModel().isModelTypeAirbot()) {
        adapter.vacbot.run('SetFanSpeed', adapter.cleanSpeed);
    } else {
...
function cleanSpotArea(adapter, mapID, mapSpotAreaID) {
    ...
    } else if (Number(mapID) === Number(adapter.currentMapID)) {
        adapter.log.info('Start cleaning spot area: ' + mapSpotAreaID + ' on map ' + mapID);
        adapter.vacbot.run('spotArea', 'start', mapSpotAreaID);
        if (adapter.spotAreaCleanings > 1) {
            adapter.cleaningQueue.createMultipleCleaningsForSpotArea('control', 'spotArea', mapSpotAreaID);
        }
    } else {
...
function goToPosition(adapter, goToPositionValues, doNotSave = false) {
    ...
    if (helper.positionValueStringIsValid(goToPositionValues)) {
        if (adapter.getModel().isModelTypeAirbot()) {
            ...
            adapter.vacbot.run('SinglePoint_V2', goToPositionValues);
        } else if (adapter.getDevice().useNativeGoToPosition()) {
            adapter.log.info(`Go to position: ` + goToPositionValues);
            adapter.vacbot.run('GoToPosition', goToPositionValues);
        } else {
