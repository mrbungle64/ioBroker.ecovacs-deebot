        ...
        switch (stateName) {
            ...
            case 'goToPosition_saveCurrentDeebotPosition': {
                ...
                if (deebotPositionState && deebotPositionState.val) {
                    mapHelper.saveGoToPositionValues(adapter, deebotPositionState.val);
                }
                break;
            }
        }
...
function goToSavedPosition(adapter, id) {
    const pattern = /control\.extended\.savedGoToPositionValues\.goToPosition_[0-9]{10}$/;
    if (pattern.test(id)) {
        adapter.getObjectAsync(id).then(obj => {
        ...
        if (doNotSave === false) {
            adapter.getStateAsync('control.extended.goToPosition_saveNextUsedValues').then(state => {
                if (state && (state.val === true)) {
                    mapHelper.saveGoToPositionValues(adapter, goToPositionValues);
                }
            });
        }
